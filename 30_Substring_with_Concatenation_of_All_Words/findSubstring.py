...
class Solution(object):
    def findSubstring(self, s, words):
        ...
        res = []
        def dfs(word, com):
            if len(com) == len(words) * len(words[0]):
                if com not in result: result.append(com)
                return 

            for idx, w in enumerate(word):
                dfs(word[0:idx] + word[idx+1:], com + w)

        dfs(words, '')
        for strr in result:
            res += [m.start() for m in re.finditer('(?=' + strr + ')', s)]
        
        return res

    def combination(self, words):
        result = []
        def dfs(word, s):
